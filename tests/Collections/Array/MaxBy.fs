  ...
  let tests =
    testList "Array.MaxBy" [
      testCase "MaxBy works as expected with negative infinity" <| fun _ ->
        ...
        let max = FSharp.Core.Extended.Collections.Array.maxBy id xs
      ...
      testCase "MaxBy throws an exception as expected with an empty array" <| fun _ ->
        let xs = [||]
        Expect.throwsT<ArgumentException> (fun () -> FSharp.Core.Extended.Collections.Array.maxBy id xs) "Expected maxBy to throw an exception for empty array"
      ...
      testPropertyWithConfig Common.config "FSharp.Core, Enumerable.Max and FSharp.Core.Extended Array.Max produce the same results" <| fun (xs: int[]) ->
          ...
          if xs.Length = 0 then
            ...
            let maxDefault = Microsoft.FSharp.Collections.Array.maxBy id xs
            let maxExtended = FSharp.Core.Extended.Collections.Array.maxBy id xs
            let maxLinq = Enumerable.MaxBy(xs, id)
